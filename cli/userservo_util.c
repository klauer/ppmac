...
bool disable_isr(unsigned char isr)
{
    ...
    pshm->Motor[isr].ServoCtrl = 0;  // stop executing user Servo interrupt
...
bool enable_isr(unsigned char isr)
{
    ...
    pshm->Motor[isr].ServoCtrl = 1;  // start executing Servo code
...
bool load_isr_function_from_addr(unsigned long addr, unsigned char isr)
{
    ...
    pshm->UserAlgo.ServoCtrlAddr[isr] = (unsigned int)addr;
    // pshm->Motor[isr].Ctrl = (PUserCtrl) addr;
    printf("Loaded OK\n");
    return true;
}
...
printusage:
    printf("User servo loading tool\n");
    printf("%s [-l/-e/-d] number [function_name]\n", argv[0]);
    printf("Examples:\n");
    printf("    Load function to UserAlgo.ServoCtrlAddr 1: %s -l 1 function_name\n", argv[0]);
    // printf("    Use UserAlgo.ServoCtrlAddr[1] on motor 3: %s -m 3 1\n", argv[0]);
    printf("    Enable motor 3 servo: %s -e 3\n", argv[0]);
    printf("    Disable motor 3 servo: %s -d 3\n", argv[0]);
    if (initialized)
        CloseLibrary();
